...
function clearAndRenderCanvasElements() {
  console.log("Clearing and Rendering the canvas");
  cleanCanvas();
...
function createElement(x1, y1, x2, y2) {
  return { x1, x2, y1, y2, element: generator.line(x1, y1, x2, y2) };
}

function cleanCanvas() {
  ctxContext.clearRect(0, 0, canvas.width, canvas.height);
}
canvas.addEventListener("mousedown", (event) => {
  ...
  requestAnimationFrame(clearAndRenderCanvasElements);
});
canvas.addEventListener("mousemove", (event) => {
  ...
  elements.splice(elements.length - 1, 1, updatedElement);
